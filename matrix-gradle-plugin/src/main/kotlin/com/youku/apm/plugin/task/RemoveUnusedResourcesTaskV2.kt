...
import com.youku.apm.arscutil.ArscUtil
import com.youku.apm.arscutil.io.ArscReader
import com.youku.apm.arscutil.io.ArscWriter
import com.youku.apm.javalib.util.Log
import com.youku.apm.javalib.util.Pair
import com.youku.apm.javalib.util.Util
import com.youku.apm.plugin.compat.AgpCompat
import com.youku.apm.plugin.compat.CreationConfig
import com.youku.apm.plugin.extension.ApmRemoveUnusedResExtension
import com.youku.apm.shrinker.ApkUtil
import com.youku.apm.shrinker.ProguardStringBuilder
...
abstract class RemoveUnusedResourcesTaskV2 : DefaultTask() {
    ...
    fun removeResources() {
        ...
        variant.outputs.forEach { output ->
            ...
            removeResourcesV2(
                project = project, originalApkFile = output.outputFile, signingConfig = AgpCompat.getSigningConfig(variant), nameOfSymbolDirectory = AgpCompat.getIntermediatesSymbolDirName()
            )
    ...
    class FindOutUnusedResources(
        private val project: Project, private val pathOfApkChecker: String, private val configOfIgnoredResources: Set<String>, private val findOutDuplicates: Boolean
    ) {
        fun exec(
            pathOfOriginalApk: String, pathOfMapping: String, pathOfRTxt: String, resultOfUnused: MutableSet<String>, resultOfDuplicates: MutableMap<String, MutableSet<String>>
        ) {
    ...
    private fun saveShrunkRTxt(
        pathOfBackup: String, mapOfResources: Map<String, Int>, mapOfResourcesGonnaRemoved: Map<String, Int>, mapOfStyleables: MutableMap<String, Array<Pair<String, Int>>>
    ) {
    ...
    private fun writeResGuardMappingFile(
        pathOfBackup: String, mapOfObfuscatedNames: HashMap<String, String>, mapOfObfuscatedDirs: HashMap<String, String>, mapOfObfuscatedFiles: HashMap<String, String>
    ) {
    ...
    private fun removeResourcesV2(
        project: Project, originalApkFile: File, signingConfig: SigningConfig?, nameOfSymbolDirectory: String
    ) {
        
        // Prepare backup dir for un-shrunk apk
        val fileOfBackup = File(project.buildDir, "outputs").resolve(BACKUP_DIR_NAME).resolve(originalApkFile.name.substring(0, originalApkFile.name.indexOf(".")))
        ...
        val rTxtFile = File(project.buildDir, "intermediates").resolve(nameOfSymbolDirectory).resolve(variant.name).resolve("R.txt")
        
        val mappingTxtFile = File(project.buildDir, "outputs").resolve("mapping").resolve(variant.name).resolve("mapping.txt")
        ...
        FindOutUnusedResources(
            project = project, pathOfApkChecker = pathOfApkChecker, configOfIgnoredResources = rulesOfIgnore, findOutDuplicates = isDeleteDuplicatesEnabled
        ).exec(
            pathOfOriginalApk = originalApkPath, pathOfMapping = mappingTxtFile.absolutePath, pathOfRTxt = rTxtFile.absolutePath,
            
            // result
            resultOfUnused = setOfUnusedResources, resultOfDuplicates = mapOfDuplicatedResources
        )
        ...
        val mapOfResources: MutableMap<String, Int> = HashMap() // Map of styleables parsed from R.txt
        ...
        calculateResources(
            setOfUnusedResources = setOfUnusedResources, mapOfDuplicatedResources = mapOfDuplicatedResources, mapOfResources = mapOfResources,
            
            // result
            resultOfResourcesGonnaRemoved = mapOfResourcesGonnaRemoved, resultOfDuplicatesReplacements = mapOfDuplicatesReplacements, resultOfObfuscatedNames = mapOfObfuscatedNames
        )
        ...
        if(removeSucceed) {
            ...
            // Save shrunk R.txt
            saveShrunkRTxt(
                pathOfBackup = fileOfBackup.absolutePath, mapOfResources = mapOfResources, mapOfStyleables = mapOfStyleables, mapOfResourcesGonnaRemoved = mapOfResourcesGonnaRemoved
            )
            
            // Write resource mapping to resguard-mapping.txt
            if(isResGuardEnabled) {
                writeResGuardMappingFile(
                    pathOfBackup = fileOfBackup.absolutePath, mapOfObfuscatedNames = mapOfObfuscatedNames, mapOfObfuscatedDirs = mapOfObfuscatedDirs, mapOfObfuscatedFiles = mapOfObfuscatedFiles
                )
            }
        }
    }
